...
const Customer = () => {
    ...
    const [customerInputData, setCustomerInputData] = useState<object>({
        name: "",
        phone: "",
        email: "",
        address: "",
    ...
    const handleSubmit = async (e: any) => {
        e.preventDefault()
        await addDoc(customerCollectionRef, customerInputData)
        .then(() => {
            successAdd()
            setTimeout(() => {
    ...
    const getCustomerData = async() => {
        ...
        .then((res) => {
            setCustomerData(res.docs.map(doc => {
                        ...
                        <div className="table-responsive">
                            <table className="table table-striped">
                                <thead className="thead-light">
                                    <tr>
                                        <th onClick={() => {console.log(customerData)}}>#</th>
                                        <th>Nama Customer</th>
                                        <th>Handphone</th>
                                        <th>Alamat</th>
                                        <th>Email</th>
                                ...
                                <tbody>
                                    {customerData && customerData.map((item:any, idx:number) => {
                                        return (
                                            <tr key={item[1]}>
                                                <td>{idx + 1}</td>
                                                <td>{item[0].name}</td>
                    ...
                    <div className="modal-dialog">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title" id="exampleModalLabel">Tambah Customer</h5>
                                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" />
                            </div>
                            <form action="#" onSubmit={(e) => handleSubmit(e)}>
                                <div className="modal-body">
                                    <div className="form-group">
                                        <input type="text" placeholder="Nama Customer" required className="form-control"
                                            onChange={(e) => {
                                                setCustomerInputData((prev: object) => ({
                                                    ...prev,
                                                    name: e.target.value
                                                }))
                                            }}
